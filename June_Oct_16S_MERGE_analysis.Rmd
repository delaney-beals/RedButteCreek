...
```{r}
envo <- bact_sample_info_table %>%
  select(flux, soil.temp, longitude)


en_pcoa <- envfit(bact_vst_pcoa, envo, permutations = 77)

vector_scores <- scores(en_pcoa, choices = c(1, 2), display = "vectors")



ordination_scores <- scores(bact_vst_pcoa, display = "sites")
...
```{r}
env <- bact_tbl %>%
  select(sample, flux, soil.temp, longitude) %>%
  column_to_rownames("sample") 
...
ggplot(data = metadata_NMDS, aes(x = NMDS1, y = NMDS2)) +
  ...
  my_theme +
  guides(fill = guide_legend(override.aes = list(color = fill_col))) +
   geom_segment(data = en_coord_cont, aes(x = 0, y = 0, xend = NMDS1/2, yend = NMDS2/2), # NMDS/2 to scale down arrow length
