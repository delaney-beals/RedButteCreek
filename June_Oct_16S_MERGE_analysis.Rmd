...
```{r}
env <- bact_tbl %>%
  select(sample, month, soil, template, flux, latitude, longitude, soil.temp) %>%
  column_to_rownames("sample") 
...
ggplot(data = data.scores, aes(x = NMDS1, y = NMDS2)) + 
     geom_point(data = data.scores, aes(colour = month), 
                size = 3, alpha = 0.5) + 
...
# save the entire table as a .csv
write.csv(deseq_res_June_vs_Oct_g, "sig_table_month_gDNA_all.csv")
...
write.csv(sig_tab_month_g, "sig_table_month_gDNA.csv")
```

One Methylotenera and one Methyloversatilis.

```{r}
deseq_res_June_vs_Oct_g_df <- as.data.frame(deseq_res_June_vs_Oct_g)
JO_g_df <- deseq_res_June_vs_Oct_g_df[complete.cases(deseq_res_June_vs_Oct_g_df), ]
...
# if log2Foldchange > 1 and pvalue < 0.001, set as "UP" 
JO_g_df$diffexpressed[JO_g_df$log2FoldChange > 9 & JO_g_df$padj < 0.01] <- "UP"
# if log2Foldchange < -1 and pvalue < 0.001, set as "DOWN"
JO_g_df$diffexpressed[JO_g_df$log2FoldChange < -9 & JO_g_df$padj < 0.01] <- "DOWN"
...
write.csv(JO_g_df_with_tax, "sig_table_month_gDNA_tax.csv")
```
...
# save the entire table as a .csv
write.csv(deseq_res_rip_vs_up_g, "sig_table_soil_gDNA_all.csv")
...
write.csv(sig_tab_soil_g, "sig_table_soil_gDNA.csv")
```
...
# if log2Foldchange > 1 and pvalue < 0.001, set as "UP" 
RU_g_df$diffexpressed[RU_g_df$log2FoldChange > 10 & RU_g_df$padj < 0.000001] <- "UP"
# if log2Foldchange < -1 and pvalue < 0.001, set as "DOWN"
RU_g_df$diffexpressed[RU_g_df$log2FoldChange < -10 & RU_g_df$padj < 0.000001] <- "DOWN"
...
write.csv(RU_g_df_with_tax, "sig_table_soil_gDNA_tax.csv")
```
...
### MOB and NMH diffential abundance analysis


Starting with our gDNA-only DESeq results, we will then filter for only our ASVs of interest
