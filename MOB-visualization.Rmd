...
## Bubble plot: MOB/NMH relative abundance
 Resource: <http://www.sthda.com/english/wiki/ggplot2-facet-split-a-plot-into-a-matrix-of-panels>
...
## Heatmap of MOB relative abundance
```{r}
# Assuming you have a data frame 'df' with the first column being the taxa
# and the remaining columns being the relative abundances for each sample.
# Sample names in the column names should include 'g' or 'c' to indicate gDNA or cDNA.
MOB_NMH1 <- read.table("data/prop.csv", header = T,  sep = ",")
# change first column name
names(MOB_NMH1)[1] <- "Sample"


# pivot to long format
data_long <- pivot_longer(
  MOB_NMH1,
  cols = -Sample, # Exclude the column that should not be pivoted
  names_to = "Genus", # This will be the new column for the library types
  values_to = "Relative_Abundance" # This will be the new column for the values
)

# Convert Genus to factor and order it based on Relative_Abundance
data_long$Genus <- factor(data_long$Genus, levels = unique(data_long$Genus[order(data_long$Relative_Abundance)]))

# Separate the library type into template, month, soil types
data_long1 <- data_long %>%
  mutate(
    Template = ifelse(startsWith(as.character(Genus), "c"), "cDNA", "gDNA"),
    Genus = gsub("^[cg]", "", Genus), # Remove c or g from the start
     # Classify as cDNA or gDNA
    Month = ifelse(startsWith(Sample, "6"), "June", 
            ifelse(startsWith(Sample, "10"), "October", NA_character_)), # Assign month
    Soil = case_when(
      grepl("U", Sample) ~ "upland",
      grepl("R", Sample) ~ "riparian",
      TRUE ~ NA_character_), 
    MonthSoil = factor(paste(Month, Soil))
  )



# Now create the heatmap 
ggplot(data_long1, aes(x = Month, y = Genus, fill = Relative_Abundance)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "blue", high = "red") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Sample", y = "Genus", fill = "Relative Abundance") + facet_wrap(~ Sample, scales = "free_x", ncol = 6)

ggplot(data_long1, aes(x = Sample, y = Genus, fill = Relative_Abundance)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "blue", high = "red") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Sample", y = "Genus", fill = "Relative Abundance") +
  facet_grid(Soil ~ Month, scales = "free_x", space = "free_x")

ggplot(data_long1, aes(x = MonthSoil, y = Genus, fill = Relative_Abundance)) +
  geom_tile(color = "white") + # Draw the tiles
  facet_wrap(~ Template, scales = "free_y", nrow = 2) + # Separate facets for gDNA and cDNA
  scale_fill_gradient(low = "blue", high = "red") + # Gradient fill based on abundance
  theme_minimal() + # Minimal theme
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1), # Angle the x-axis text
    axis.text.y = element_text(size = 7) # Adjust size of y-axis text if needed
  ) +
  labs(x = "Month and Soil Type", y = "Genus", fill = "Relative Abundance") # Labels



# try again
data_long2 <- data_long1 %>%
  unite("MonthSoilTemplate", Month, Soil, Template, sep = "_", remove = FALSE)

# Now plot with ggplot2 using the updated MonthSoilTemplate for x-axis positioning
ggplot(data_long2, aes(x = MonthSoilTemplate, y = Genus, fill = Relative_Abundance)) +
  geom_tile(color = "white") + # Draw the tiles
  scale_fill_gradient(low = "blue", high = "red") + # Gradient fill based on abundance
  theme_minimal() + # Minimal theme
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 7), # Angle the x-axis text and adjust size
    axis.text.y = element_text(size = 7) # Adjust size of y-axis text if needed
  ) +
  labs(x = "Month, Soil Type, and DNA Type", y = "Genus", fill = "Relative Abundance") # Labels

# transform rel abund to log scale
data_long2 <- data_long2 %>%
  mutate(Relative_Abundance_Log10 = log10(Relative_Abundance + 1))

ggplot(data_long2, aes(x = MonthSoilTemplate, y = Genus, fill = Relative_Abundance_Log10)) +
  geom_tile(color = "white") + # Draw the tiles
  scale_fill_gradient(low = "blue", high = "red") + # Gradient fill based on abundance
  theme_minimal() + # Minimal theme
  theme(
  ...
  labs(x = "Month, Soil Type, and DNA Type", y = "Genus", fill = "Relative Abundance")
```
```{r}
subset_data <- data_long2 %>%
  filter(Genus %in% c("Methylobacter", "Methylomonas"), !MonthSoil %in% c("October upland"))

ggplot(subset_data, aes(x = MonthSoilTemplate, y = Genus, fill = Relative_Abundance_Log10)) +
  geom_tile(color = "white") + # Draw the tiles
  scale_fill_gradient(low = "blue", high = "red") + # Gradient fill based on abundance
  theme_minimal() + # Minimal theme
  theme(
...
# correlations between gDNA and cDNA
```{r}
MOB_NMH <- read.table("data/prop.csv", header = T, row.names = 1, sep = ",")
...
cc_pears_abund = cor(abun, method = "pearson")

# make plot
corrplot(cc_pears_abund, tl.col = "black", order = "hclust", hclust.method = "average", addrect = 4, tl.cex = 0.7)

corrplot(cc_pears_abund, tl.col = "black",addrect = 4, tl.cex = 0.7,type = "upper")
...
# keep cDNA on top and gDNA on left
corr_df <- Hmisc::rcorr(as.matrix(MOB_NMH), type=c("pearson"))


# plot only the correlations we want
corrplot(corr=corr_df$r[18:30, 1:17], 
         p.mat=corr_df$P[18:30, 1:17], 
         tl.col = "black",
         tl.srt = 45,
         insig = "pch", 
         sig.level = c(.05), 
...
In cases where the gDNA of a member doesn't correlate with the cDNA abundance of the same member (which is most taxa in the plot), we can think of this as just because an individual is present (gDNA), doesn't mean we automatically see higher transcripts in cDNA. So this gives us more support for looking into the differences between gDNA and cDNA for individuals. 

The presence of Flavobacterium is correlated with the activity of Methylobacter, Methylomonas, Methylophilus, Flavobacterium, etc. But does this mean that the presence of Methylobacter for example doesn't correlate with the activity of anything else? Meaning Methylobacter presence doesn't potentially influence methylotroph/crossfeeding members? Basically means that just because Methylobacter is there, doesn't mean it is correlating with methylotroph activity, we might have to have Methylobacter also be active in order for the crossfeeder's activity to also be different; which makes sense since Methylobacter has to produce byproducts/mystery signals for the partner to interact with and if Methylobacter is dormant, the NMM partner is going to do it's own thing. 

Looking at this from the other direction, the activity of Methylobacter correlates with the presence of Flavobacterium and Methyloligella. These might be potential crossfeeders who rely on the byproducts of Methylobacter to establish themselves. 
...
# re-order genera
desired_order2 <- c("Methylomonas","Methylobacter","Methyloferula", "Methylophilus", "Methylorubrum","Methyloversatilis", "Methylobacterium", "Methylotenera", "Methyloceanibacter", "Methylibium", "Methyloligella","Methylopila", "Acidovorax", "Comamonas", "Flavobacterium", "Methylomicrobium","Methylosinus") 

new_comc <- comc[, desired_order2]
new_comc = new_comc[,1:15]
...
new_comc_corr <- Hmisc::rcorr(as.matrix(new_comc), type=c("pearson"))


# plot only the correlations we want
corrplot(corr=new_comc_corr$r[1:15, 1:15], 
         p.mat=new_comc_corr$P[1:15, 1:15], 
         tl.col = "black",
         tl.srt = 45,
         insig = "pch", 
         sig.level = c(.05), 
